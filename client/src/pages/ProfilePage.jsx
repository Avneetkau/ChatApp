...
const ProfilePage = () => {
  ...
  //Step-1 Here, we will first check authauser is true or not if, true then updateProfile will run
    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      ...
      justify-between max-sm:flex-col-reverse rounded-lg '>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor='avatar' className='flex items-center gap-3 cursor-pointer'>
           ...
           <input
            ...
            type='text' placeholder='Name' className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2' required/>
            <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
              placeholder='Write bio here...' required
        ...
        <img src={authUser?.profilePic || assets.logo_icon}
         className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 ${selectedImg && 'rounded-full'}`} alt=''/>
      </div>
    </div>
  );
}
